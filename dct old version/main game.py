# WIP: NEED TO REPLACE ALL rooms[currentRoom] WITH location
# WIP: currently working toward the journal with the "note" "item" on "lamp" object
...
def show_location():
    ...
    if "item" in location:
        print("You see a %s." % ", ".join(location["item"].keys()))
...
def convert_items(obj):
    it_lst = loc_obj[obj]["item"]
    if isinstance(it_lst, list):
        dct_lst = {}
        for dct in it_lst:
            dct_lst[dct["it name"]] = dct
        loc_obj[obj]["item"] = dct_lst


# converts objects in rooms from list to dict
# two occurrences: beginning of game and whenever player enters a room
def convert_objects():
    ...
    if action[0] == "go":
        ...
        if action[1] in location:
            ...
            # if door is closed/unlocked
                ...
                ask = yes_or_no()
                if ask:
                    location[action[1]]["status"] = "opened"
